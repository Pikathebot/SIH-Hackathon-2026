# Official Problem Statement: SatQuery AI
...
| **Training / Fine-Tuning Dataset** | **BigEarthNet.txt** — primary dataset for remote-sensing adaptation using co-registered Sentinel-1 SAR, Sentinel-2 multispectral imagery, and diverse text annotations. Link: [https://arxiv.org/abs/2603.29630](https://arxiv.org/abs/2603.29630). All datasets are available online open source. |
| **Public Evaluation Benchmarks** | **VRSBench** — for remote-sensing image captioning, visual question answering, and visual grounding.<br>**RSVQA** — for remote-sensing visual question answering.<br>**CDVQA** — for change-based visual question answering on bitemporal imagery. |
...
## 6. Agentic Model and Tool Orchestration
...
5. **Combine textual and spatial outputs**, estimate confidence, and return visual evidence; and
6. **Provide an auditable execution summary** containing the selected task, model/tool names, and key parameters.

*Note: The controller may perform internal task planning; however, only the observable execution trace, including the selected task, models or tools, permitted parameters, and outputs will be evaluated. Internal reasoning text is neither required nor evaluated.*
...
- Code and models including test suite and demonstration.
...
- **Public Benchmarks**: Evaluated using prescribed test splits (VRSBench, RSVQA, CDVQA).
- **ISRO/SAC Evaluation Set**: Pre-georeferenced and co-registered Cartosat-2S optical and RISAT SAR image pairs, with task-specific reference answers, labels, bounding boxes, or masks, as applicable. Evaluation annotations will not be disclosed to participating teams.
