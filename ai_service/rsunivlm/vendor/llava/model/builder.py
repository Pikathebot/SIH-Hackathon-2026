...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            # token_num = llava_cfg.vocab_size
            if model.lm_head.weight.shape[0] != token_num:
                model.lm_head.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=model.device, dtype=model.dtype))
                model.model.embed_tokens.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=model.device, dtype=model.dtype))
                # tokens_dict = ["[VG]", "[CCD]", "[VQA]", "[CAP]", "[CLS]"]
                # for x in range(1000):
                #     tokens_dict.append(f'<loc_{x}>')
                # num_new_tokens = tokenizer.add_tokens(tokens_dict)
            ...
            if "mixtral" in model_name.lower():
                ...
                if "gmoe" in  model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        model = LlavaQwenMoeForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation,trust_remote_code=True, **kwargs)
