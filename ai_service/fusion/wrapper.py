...
def run_fusion(
    ...
    try:
        ...
        # Visible Green Vegetation Index (VDVI / GLI proxy)
        eps = 1e-6
        green_index = (2 * G - R - B) / (2 * G + R + B + eps)
        water_index = (B - R) / (B + R + eps)
        brightness = (R + G + B) / 3.0

        opt_veg_mask = green_index > 0.08
        opt_water_mask = (water_index > 0.05) & (brightness < 0.45)

        # ── 2. SAR Radar Backscatter Analysis ────────────────────────
        # High backscatter (> 0.62): Double-bounce corner reflectors (built-up, concrete, metal)
        # Low backscatter (< 0.22): Specular reflection (calm water bodies, flat paved surfaces)
        # Medium backscatter (0.22 - 0.62): Volume scattering (canopy, vegetation roughness)
        sar_high_backscatter = sar_np > 0.60
        sar_specular_water = sar_np < 0.22
        sar_volume_veg = (sar_np >= 0.22) & (sar_np <= 0.60)
        ...
        # Fused Water: both optical water and SAR specular agree OR strong spectral water
        fused_water = (opt_water_mask & sar_specular_water) | (water_index > 0.15)
        # Fused Vegetation: optical greenness confirmed by SAR roughness
        fused_veg = (opt_veg_mask & sar_volume_veg) | (green_index > 0.12)
        # Built-up: SAR double bounce OR high optical structural texture
        fused_urban = sar_high_backscatter | (~fused_water & ~fused_veg)
        ...
        final_urban_pct = 100.0 - final_water_pct - final_veg_pct
